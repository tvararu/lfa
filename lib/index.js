...
function create_toc(root) {
  ...
  function parse_locals(filepath) {
    ...
    // we need this function while calculating some md5 hashes for articles and sections.
    ...
    var html = jade.compile(content, { compileDebug: false })(dummy);
    dummy.html = html;
...
function compile(ast){
  ...
  function copy_components(cb) {
    if (global.options.components == "false") {
      ...
      shell.cp(
        '-rf', 
        path.join(__dirname, '../patterns/lfa-components/'), 
    ...
    mkdirp(path.join(path.normalize(process.cwd()), options.output_folder, '/js'), function(err) {
      fs.openSync(path.join(path.normalize(process.cwd()), options.output_folder, '/js/main.js'), 'a');
    });
